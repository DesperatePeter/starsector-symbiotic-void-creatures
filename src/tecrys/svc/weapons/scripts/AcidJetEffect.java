...
public class AcidJetEffect extends CryofluxTransducerEffect {
	...
	public AcidJetEffect() {
		super();
	}

	public AcidJetEffect(DamagingProjectileAPI projectile, DamagingProjectileAPI prev) {
		...
		float size = proj.getProjectileSpec().getWidth() * 0.4f;
		...
		for (ParticleData p : negativeParticles) {
			...
			p.scaleIncreaseRate *= 3f + (float) Math.random();
...
//			Vector2f.add(p.offset, dir, p.offset);
	...
	protected AcidJetEffect createTrail(DamagingProjectileAPI projectile, DamagingProjectileAPI prev) {
		return new AcidJetEffect(projectile, prev);
	}
